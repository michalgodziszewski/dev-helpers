...
## Workflow H: autonomous run with --yolo

Use `--yolo` to run a fully specified spec from load all the way to the push, without stepping through each action manually. It suits work whose spec already carries workflow, base branch, goals, and scope.

```text
/feature load context/features/0015-feature-load-yolo-flag.md --yolo
```

The flag works on every load form:

```text
/feature load trunk feature --ticket LSG-12345 --yolo "Add account summary"
/feature load branch release-1.80.0 fix --yolo payment-timeout
```
...
|---|---|
| Run a fully specified spec straight through to the push | `load ... --yolo` |
