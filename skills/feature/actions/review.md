...
8. If no code-review subagent is available in the project (Kiro has no on-demand delegation mechanism, so this always applies there), do not fail. Continue with the own analysis above; when a stack-specific guidance steering file is installed (e.g. `.kiro/steering/feature-code-review-guidance.md`), read it and apply its checklist during the own-analysis pass instead of a generic review. Clearly note whether the code-quality pass was delegated to a subagent, ran against installed stack-specific guidance, or was skipped entirely because neither is available.
