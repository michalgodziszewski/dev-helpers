...
Use context/current-feature.md as ignored personal runtime state. If it does not exist, create it from ../../../skills/feature/assets/current-feature-template.md. That template is placeholder-based; when bootstrapping runtime state, drop the preview notice and the planning-only sections and clear remaining placeholders as described in ../../../skills/feature/actions/load.md.
...
When History grows past 10 entries, complete rotates the older ones verbatim into context/history-archive.md (archive is kept newest-first), an ignored personal archive file that is never required to exist. See ../../../skills/feature/actions/complete.md.
...
Use context/feature-config.md as ignored personal configuration. If it does not exist, create it with the default configuration content defined in ../../../skills/feature/actions/load.md. Never stage or commit it.
...
The shared ../../../skills/feature/assets/ directory (provider-neutral, used by every installed provider) contains only current-feature-template.md — the source of truth for runtime state bootstrapping and for specs generated by the plan action, using {{...}} placeholders, optional References/Scope/Documentation Requirements sections, a removable preview-notice marker block, and a skill-managed History.

Every other template lives in the custok-workflow package root under assets/ and is installed into a project by the dev feature-skill-install (Claude Code) or dev feature-skill-install-kiro (Kiro) CLI commands:
...
Each installer asks which coding-standards and code-review-guidance stack to use — it never infers the stack or silently selects a default — ensures /context/ is present in .gitignore (or the provider-equivalent ignore mechanism), and never stages generated context files. In an installed project the skill relies on those installer-scaffolded context files; it does not require access to the package root assets/.

When creating a new Markdown specification by hand, copy the structure of feature-spec-template.md (installed projects can use any existing spec in context/features/ as reference) into context/features/ or context/fixes/ and replace every placeholder. Never edit a template for a single work item. The plan action instead generates specs from ../../../skills/feature/assets/current-feature-template.md.
...
This section applies to Claude Code, which has an on-demand Agent tool. Delegate read-only or self-contained work to the installed subagents via the Agent tool. When an agent is not installed, run the same procedure inline and note that delegation was skipped — never fail because an agent is missing:
...
Detailed English documentation lives in ../../../skills/feature/docs/ (shared across every installed provider):

- ../../../skills/feature/docs/README.md for the documentation map, guarantees, prerequisites, and quick start
- ../../../skills/feature/docs/configuration.md for local context, Jira policy, templates, and specification metadata
- ../../../skills/feature/docs/state-model.md for active statuses, Pending Reviews, History, and atomic metadata
- ../../../skills/feature/docs/workflows.md for trunk, explicit-base, review queue, completion, and backport flows
- ../../../skills/feature/docs/action-reference.md for exact syntax, preconditions, effects, and stop conditions
- ../../../skills/feature/docs/planning.md for the iterative plan workflow, preview specs, numbering, screenshots, and finalization
- ../../../skills/feature/docs/jira-naming.md for ticket validation and branch/commit rendering
- ../../../skills/feature/docs/scenarios.md for end-to-end examples across common and failure situations
- ../../../skills/feature/docs/safety-and-recovery.md for destructive boundaries, conflicts, and recovery
- ../../../skills/feature/docs/troubleshooting.md for symptom-driven diagnostics
...
Read and execute only the matching file under ../../../skills/feature/actions/. If no action is provided, show the action table and examples. Do not commit, push, merge, cherry-pick, delete a branch, or reset state unless the matching action explicitly authorizes it.
